...
export default class RestClient {
  ...
  public bootstrap(manager: Manager): AxiosInstance {
    ...
    const env = manager.getConfig<{ MFOX_API_URL: string }>('root.env');

    const instance = axios.create({
      baseURL: window.__api_url__ || env?.MFOX_API_URL || '/api/v1'
    });
    ...
    instance.interceptors.response.use(
      ...
      error => {
        if (error?.response) {
          ...
          if (statusCode === 401 && !config._retry && manager.dispatch) {
            config._retry = true;
