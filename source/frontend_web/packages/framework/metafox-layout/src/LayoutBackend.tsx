...
export default class LayoutBackend {
   ...
   * List of key sizes
   */
  private allSizes: PageSize[] = ['small', 'medium', 'large'];
  /**
   * + Manager
   */
  ...
  constructor(config: Partial<LayoutBackendConfig>) {
    if (config.pages) this.pageLayouts = config.pages;

    if (config.siteDock) this.siteDock = config.siteDock;
  }
  ...
  public resolve({
    ...
    if (!hasAdminAccess && MFOX_BUILD_TYPE === 'admincp') {
    ...
    if (offline && MFOX_BUILD_TYPE === 'web') {
      name = 'core.offline';
    }
    ...
    const pageSize = this.getApproximatePageSize(
      name,
    ...
    const templateName =
      this.editingTemplateName ?? pageLayout.templateName ?? 'default';
    ...
    blocks = blocks.filter(block => block && block.slotName);
  ...
  public updateBlockPosition(props: {
    ...
    const { blockId, order, slotName, pageName, pageSize } = props;
    ...
    blocks
      .filter(block => block.slotName === slotName)
      .map((block, index) => ({
  ...
  public setupTheme({ info, default: config }: LoadedTheme) {
    ...
    if (config.siteBlocks) {
      this.siteBlocks = config.siteBlocks;
    }

    this.gridPresets = config.gridLayouts;
    this.blockPresets = config.blockLayouts;
