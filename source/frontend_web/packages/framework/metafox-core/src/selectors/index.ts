...
const getPollAnswers = (state: GlobalState, identities: string[]) => {
...
const getSession = (state: GlobalState) => state.session;
...
export const getDeepItemSelector = createSelectorCreator(
  defaultMemoize,
  isEqual
);
...
export const getPhotosSelector = createSelector(getItems, data => data);
