...
import { IS_INVITED } from '@metafox/pages';
...
const InviteCard = ({ item, actions }) => {
  ...
  if (item?.membership !== IS_INVITED) return null;

  if (!user || !invite) return null;

  const { invited_admin, expired_description } = invite;

  let messageInvite = 'person_who_invite_me_page';

  if (invited_admin) {
    messageInvite = 'person_who_invite_me_to_admin_page';
  }
