...
function* pinItem({ payload: { identity } }: ItemLocalAction) {
  ...
  const { apiClient, compactUrl, getPageParams, getSetting, i18n } =
    yield* getGlobalContext();
  const limit = parseInt(getSetting('activity.feed.total_pin_in_profile'));
  const { profile_id: owner_id } = getPageParams<{ profile_id?: string }>();
  ...
  try {
    ...
    if (isOverLimit) {
      yield put({
        ...
        payload: {
          data: [identity],
          pagingId: [`/feed?user_id=${owner_id}`]
        }
      });
    } else {
      yield put({
...
function* unpinItem({ payload: { identity } }: ItemLocalAction) {
  const item = yield* getItem(identity);
  const { apiClient, compactUrl, getPageParams, dialogBackend, i18n } =
    yield* getGlobalContext();
