...
export default function ImageView({ image, images }: IProps) {
  ...
  const downloadImage = (src: string, filename?: any) => {
    if (!src) return;

    triggerClick(src, filename);
  };
      ...
      >
        ...
        <RootDialogContent dividers={false} isMobile={isMobile}>
          <DialogImage>
            ...
            {image?.download_url && image?.is_image ? (
              <IconDownLoad
                onClick={() =>
                  downloadImage(image.download_url, image.file_name)
                }
                isMobile={isMobile}
              >
            ...
            {item.id === activeImageKey ? (
              <DialogImage>
                ...
                {item?.download_url ? (
                  <IconDownLoad
                    onClick={() =>
                      downloadImage(item.download_url, item.file_name)
                    }
                    isMobile={isMobile}
                  >
                    <LineIcon icon="ico-download" />
