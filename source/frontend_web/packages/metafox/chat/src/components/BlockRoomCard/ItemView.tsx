...
const RootBuddyItem = styled(ItemView, {
  ...
  isMobile?: boolean;
  variant?: string;
}>(({ theme, unread, isActive, isMobile, variant }) => ({
  cursor: 'pointer',
...
const LastMessage = ({ item, unread }: any) => {
  ...
  let lastMessage: any = formatLastMsg(item);
      ...
      <ItemMsgText unread={unread}>
        {item && lastMessage ? (
          <>
            {user?.full_name}:{' '}
            {isArray(lastMessage) ? (
              i18n.formatMessage(lastMessage[0])
  ...
  return (
    ...
    >
        ...
        <ItemInner>
          <ItemRowTitleWrapper>
            ...
            <ItemSubtitle className="itemSubtitle">
              <FromNowChat value={lastMessageItem?.updated_at} />
            </ItemSubtitle>
          </ItemRowTitleWrapper>
      ...
      {itemProps?.showActionMenu ? (
        <ItemActionStyled visible="hover" variant={itemProps?.type}>
          <ItemActionMenu
            items={items}
