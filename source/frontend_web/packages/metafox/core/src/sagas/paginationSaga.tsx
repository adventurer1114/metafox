...
import { uniq, omit, isEqual, isEmpty } from 'lodash';
...
export function* fetchPaginationSaga({ type, payload, meta }: FetchAction) {
  ...
  try {
    ...
    const pageState = canLoadMore ? (state?.page || 0) + 1 : undefined;

    const page = isLoadMorePagination ? pageParams?.page : pageState;

    if (maxPageNumber !== 0 && page > maxPageNumber) return;
    ...
    let ended =
      0 === result.ids.length ||
      isEqual(result.ids, nextState.ids) ||
      (links && isLoadMoreButton && isEmpty(links?.next));

    if (isLoadMoreButton && numberOfItemsPerPage) {
      ended = numberOfItemsPerPage > result.ids.length;
    }
